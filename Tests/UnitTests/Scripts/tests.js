...
describe("XMLHTTPRequest", function () {
    ...
    function createRequestWithHeaders(method, url, headers, values) {
        return new Promise((resolve, reject) => {
            const xhr = new XMLHttpRequest();
            xhr.open(method, url);
            xhr.setRequestHeader(headers, values);
            xhr.addEventListener("loadend", () => resolve(xhr));
            xhr.send();
        });
    }
    ...
    it("should make a GET request with headers successfully", async function () {
        const xhr = await createRequestWithHeaders("GET", "https://httpbin.org/get", "testHeader", "testValue");
        expect(xhr).to.have.property('readyState', 4);
        expect(xhr).to.have.property('status', 200);
    });
    it("should make a POST request successfully", async function () {
        const xhr = await createRequest("POST", "https://httpbin.org/post");
        expect(xhr).to.have.property('readyState', 4);
        expect(xhr).to.have.property('status', 200);
    });
    it("should make a POST request with headers successfully", async function () {
        const xhr = await createRequestWithHeaders("POST", "https://httpbin.org/post", "testHeader", "testValue");
        expect(xhr).to.have.property('readyState', 4);
        expect(xhr).to.have.property('status', 200);
    });
});
...
describe("AbortController / AbortSignal", function () {
    ...
    // Expect aborted to be false after abort()
    controller.signal.onabort = () => {
        expect(controller.signal.aborted).to.equal(true);
    }

    it("should have aborted flag set false", function () {
        expect(controller.signal.aborted).to.equal(false);
    })

    it("should not throw while aborting", function () {
        controller.abort();
    })
})
